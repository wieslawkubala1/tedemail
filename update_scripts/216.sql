...
as
	...
	print '(last_set_lower_range < dateadd(month, -3, getdate())): ' +	case when (@last_set_lower_range < dateadd(month, -3, getdate()))  then '1' else '0' end
	print '(actual_needed_lower_range > dateadd(month, -3, getdate())) ' + case when (@actual_needed_lower_range > dateadd(month, -3, getdate()))  then '1' else '0' end
	...
	if (@last_set_lower_range_time < dateadd(day, -2, getdate())) and
			(@last_set_lower_range < dateadd(month, -3, getdate())) and
			(@actual_needed_lower_range > dateadd(month, -3, getdate()))
	begin
		update partition_info 
			set last_lower_range_change = getdate(), 
			    lower_range = dateadd(month, -3, getdate());

		print 'Wracam po dwoch dniach do zakresu zakresu ostatnich 3 miesiecy'
