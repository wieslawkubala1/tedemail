...
--		exec check_partition_lower_range @actual_needed_lower_range=@start
	...
	--		drop table #zmienne 
			...
			begin 
				...
				if @DoNotShowEmptyTracks2 = 1 
				begin 
					select  t1.*, t1.ZuzycieSum_all as summary_paliwo_sum, t1.summary_paliwo_srednio_km_all as summary_paliwo_srednio,
						...
						t2.czy_dynamiczny, sign(t2.sonda) znak from #eksport_day_work t1 left join car t4 on (t1.evb2 = t4.id_car) left join #paliwo_wynik2 t2 on (t2._117 >= t1.start_praca and t2._117 < t1.stop_postoj and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) and t1.id = (select top 1 t3.id from #eksport_day_work t3 where t2._117 >= t3.start_praca and t2._117 < t3.stop_postoj and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) and t1.id = t3.id order by t3.id))
