ALTER procedure [dbo].[sp_day_work_with_tank] 
	    @isInCarMode integer, 
	    ...
	    @no_cache integer = 1 
	as
		declare @start_date_real datetime;
		declare @stop_date_real datetime;
		if @isInCarMode = 1
		begin
			EXEC showRealTimeRangeByRoutes
				@id_car = @car_or_driver_no,
				@start_date = @start,
				@stop_date = @stop,
				@start_date_real = @start_date_real OUTPUT,
				@stop_date_real = @stop_date_real OUTPUT 

			if @start_date_real < @start
				set @start = @start_date_real;
			if @stop_date_real > @stop
				set @stop = @stop_date_real;
		end
			...
			begin 
				...
				begin 
				  set @id = null 
					...
					if @id is not null 
					begin 
						update #paliwo85 set zb1 = @last_1bak, zb2 = @last_2bak where dataczas = @evdata
		 ...
		 @old_droga_tmp, 
		  @last_groupid, 
		   @ile_postoj_tmp, 
			@ile_praca_tmp, 
			 @ile_jalowy_tmp, 
			  cast(STR(MONTH(@start_praca_tmp)) + '/' + STR(DAY(@start_praca_tmp)) + '/' + STR(YEAR(@start_praca_tmp)) as datetime), 
			   @dzien, 
			    isnull(@tmp, 0.0), 
				 isnull(@tmp2, 0.0), 
				  FLOOR(@old_jalowy_tmp / 60) * 60, 
				   DATEPART(year, @start_praca_tmp) * 100 + @tmp3, 
				    DATEPART(year, @start_praca_tmp) * 100 + DATEPART(month, @start_praca_tmp), 
					 @szerokosc, 
					  @dlugosc, 
					   @szerokosc0, 
					    @dlugosc0, 
						 @first_event_datetime_db, 
						  @last_event_datetime_db, 
						   @obroty, 
						    @obroty_count, 
							 0, 
							  0,
	...
	--		drop table #zmienne 
			...
			begin 
				declare aggr_cr5 cursor  LOCAL FAST_FORWARD
					for select evb2, min(start_praca), max(stop_praca) 
			...
			print 'uaktualnianie last_1bak uwzgledniajac wylaczenia zasilania';
			update t1
				set t1.last_1bak = t3.zb1
			from #eksport_day_work t1 
				left join #paliwo85 t2 on (t1.evb2 = t2.evb2 and t2.evb3 = 10 and t1.start_praca = t2.dataczas)
				left join #paliwo85 t3 on (t1.evb2 = t3.evb2 and t3.evb3 = 15 and t3.id = t2.id + 1)
			where t3.id is not null and t3.zb1 is not null
--			print convert(varchar(10), @@ROWCOUNT) + ' rekordow zmieniono';


			print 'uaktualnianie last_2bak uwzgledniajac wylaczenia zasilania';
			update t1
				set t1.last_2bak = t3.zb2
			from #eksport_day_work t1 
				left join #paliwo85 t2 on (t1.evb2 = t2.evb2 and t2.evb3 = 10 and t1.start_praca = t2.dataczas)
				left join #paliwo85 t3 on (t1.evb2 = t3.evb2 and t3.evb3 = 15 and t3.id = t2.id + 1)
			where t3.id is not null and t3.zb2 is not null
--			print convert(varchar(10), @@ROWCOUNT) + ' rekordow zmieniono';

/*
--select t1.start_praca, t1.stop_praca, t1.first_1bak, t1.last_1bak, t3.zb1
select t1.*, t2.*, t3.*
from #eksport_day_work t1 
	left join #paliwo85 t2 on (t1.evb2 = t2.evb2 and t2.evb3 = 10 and t1.start_praca = t2.dataczas)
	left join #paliwo85 t3 on (t1.evb2 = t3.evb2 and t3.evb3 = 15 and t3.id = t2.id + 1)
...
--select * from #eksport_day_work
			...
			update #eksport_day_work set ZuzycieSum_all = ZuzycieSum_all_route
				...
				begin 
					...
					begin 
						select t1.*, 
							...
							t1.srednio_h as motogodziny,
							t1.summary_paliwo_srednio_h_day as summary_motogodziny_day,
							t1.summary_paliwo_srednio_h_all as summary_motogodziny_all
						from #eksport_day_work t1
			...
			begin 
				...
				begin 
					select  t1.*, 
							...
							t1.summary_paliwo_srednio_h_day as summary_motogodziny_day,
							t1.summary_paliwo_srednio_h_all as summary_motogodziny_all,
							t2.sonda tankowanie, t2._117 poczatek, t2._118 koniec, t2._117 _113, t2.from_sonda, 
							t2.to_sonda, t2.from_sonda2, t2.to_sonda2, t2.from_sonda_pal, t2.to_sonda_pal, 
							t2.from_sonda2_pal, t2.to_sonda2_pal, 
							t2.czy_dynamiczny, sign(t2.sonda) znak 
					from #eksport_day_work t1 left join 
								car t4 on (t1.evb2 = t4.id_car) left join 
								#paliwo_wynik2 t2 on (t2._117 >= t1.start_praca and t2._117 < isnull(t1.stop_postoj, @stop) 
									and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) 
									and t1.id = (select top 1 t3.id 
												 from #eksport_day_work t3 
												 where t2._117 >= t3.start_praca and t2._117 < isnull(t3.stop_postoj, @stop) 
													and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) 
													and t1.id = t3.id 
												 order by t3.id))
					where (t1.stop_praca > @start) and (t1.start_praca < @stop) 
				...
				begin 
					...
					from #eksport_day_work t1 left join 
						car t4 on (t1.evb2 = t4.id_car) left join 
						#paliwo_wynik2 t2 on (t2._117 >= t1.start_praca and t2._117 < isnull(t1.stop_postoj, @stop) and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) 
							and t1.id = (select top 1 t3.id 
											from #eksport_day_work t3 
											where t2._117 >= t3.start_praca and t2._117 < isnull(t3.stop_postoj, @stop) 
											and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) and t1.id = t3.id 
											order by t3.id))
					where (t1.stop_praca > @start) and (t1.start_praca < @stop) 
					order by t1.id 
				end 
...
--select * from #paliwo85

/*
select t1.*, t2.*, t3.*
from #eksport_day_work t1 
	left join #paliwo85 t2 on (t1.evb2 = t2.evb2 and t2.evb3 = 10 and t1.start_praca = t2.dataczas)
	left join #paliwo85 t3 on (t1.evb2 = t3.evb2 and t3.evb3 = 15 and t3.id + 1 = t2.id and t1.stop_praca >= t3.dataczas)
where t3.id is not null and t3.zb1 is not null
*/
