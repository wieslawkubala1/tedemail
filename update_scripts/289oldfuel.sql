				...
				begin 
					...
					begin 
						...
						insert into #eksport_day_work (evb2, marka_pojazdu, nr_rej_pojazdu, kierowca, 
							...
							@dzien, isnull(@tmp, 0.0), isnull(@tmp2, 0.0), FLOOR(@old_jalowy_tmp / 60) * 60, 
					...
					begin 
						...
						insert into #eksport_day_work (evb2, marka_pojazdu, nr_rej_pojazdu, kierowca, 
							...
							cast(STR(MONTH(@start_praca_tmp)) + '/' + 
								 ...
								 STR(YEAR(@start_praca_tmp)) as datetime), 
							@dzien, 
							isnull(@tmp, 0.0), isnull(@tmp2, 0.0), FLOOR(@jalowy_tmp / 60) * 60, 
							DATEPART(year, @start_praca_tmp) * 100 + @tmp3, 
							DATEPART(year, @start_praca_tmp) * 100 + 
...
t1.summary_paliwo_srednio_h_all as summary_motogodziny_all,
						...
						t2.czy_dynamiczny, sign(t2.sonda) znak from #eksport_day_work t1 left join car t4 on (t1.evb2 = t4.id_car) left join #paliwo_wynik2 t2 on (t2._117 >= t1.start_praca and t2._117 < isnull(t1.stop_postoj, @stop) and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) and t1.id = (select top 1 t3.id from #eksport_day_work t3 where t2._117 >= t3.start_praca and t2._117 < isnull(t3.stop_postoj, @stop) and ((t2.sonda >= t4.I_TANK) or (t2.sonda <= (-t4.I_UP))) and t1.id = t3.id order by t3.id))
					where (t1.stop_praca > @start) and (t1.start_praca < @stop) 
					and t1.przebieg > 0.0 
					order by t1.id 
				end 
